// Verifies that updateChunks() actually builds meshes and adds them to the scene.
...
class BufferGeometry { constructor(){this.attributes={};} setAttribute(n,a){this.attributes[n]=a;} setIndex(i){this.index=i;} dispose(){} }
...
const meshes = world.getCollidableMeshes();
console.log('meshes in scene:', meshes.length);
console.log('scene children:', scene.children.length);
if (meshes.length === 0) {
  console.error('FAIL: no meshes built — world would be invisible');
  process.exit(1);
}
console.log('PASS: world meshes are built and added to scene');
